...
namespace noctalia::ipc {
  ...
  constexpr std::string_view kThemeModes[] = {"dark", "light", "auto"};
  constexpr std::string_view kBuiltinPalettes[] = {"Ayu",      "Catppuccin", "Dracula", "Eldritch",  "Gruvbox",
                                                   "Kanagawa", "Noctalia",   "Nord",    "Rosé-Pine", "Tokyo-Night"};
  constexpr std::string_view kWallpaperSchemes[] = {"m3-tonal-spot", "m3-content", "m3-fruit-salad", "m3-rainbow",
                                                    "m3-monochrome", "vibrant",    "faithful",       "soft",
                                                    "dysfunctional", "muted"};
  constexpr std::string_view kEffectsProfileKinds[] = {"output", "input"};
  constexpr std::string_view kBarLayers[] = {"overlay", "top", "bottom", "background"};
  constexpr std::string_view kBooleanStates[] = {"on", "off", "true", "false", "1", "0"};
  constexpr std::string_view kPowerProfiles[] = {"power-saver", "balanced", "performance"};
  constexpr std::string_view kLogLevels[] = {"trace", "debug", "info", "warn", "error", "off"};
  constexpr std::string_view kAutoHideStates[] = {"on", "off", "smart", "true", "false", "1", "0"};
  constexpr std::string_view kMediaActions[] = {"play", "pause", "play-pause", "stop", "next", "previous"};
  constexpr std::string_view kSessionActions[] = {"lock", "logout", "suspend", "hibernate", "reboot", "shutdown"};

  // ── Positionals ────────────────────────────────────────────────────────

  // Shared
  // msg <cmd-up/down> [step]
  constexpr CliPositional kStepOptionalPositionals[] = {
      {.name = "step", .description = "optional step value (e.g. 5 or 5%)", .required = false}
  };

  // msg cmd-osd [value]
  constexpr CliPositional kOptionalValuePositionals[] = {
  ...
  constexpr CliPositional kBarAutoHideSetPositionals[] = {
      {.name = "state",
  ...
  constexpr CliPositional kBarHidePositionals[] = {
      {.name = "bar-name", .description = "target bar name", .required = false},
      {.name = "monitor-selector", .description = "target monitor selector", .required = false}
  };

  // msg bar-layer-set <layer> [id]
  constexpr CliPositional kBarLayerSetPositionals[] = {
  ...
  constexpr CliPositional kBarReserveTogglePositionals[] = {
      {.name = "bar-name", .description = "target bar name", .required = false},
      {.name = "monitor-selector", .description = "target monitor selector", .required = false}
  };

  // msg bar-show [bar-name] [monitor-selector]
  constexpr CliPositional kBarShowPositionals[] = {
      {.name = "bar-name", .description = "target bar name", .required = false},
      {.name = "monitor-selector", .description = "target monitor selector", .required = false}
  };

  // msg bar-toggle [bar-name] [monitor-selector]
  constexpr CliPositional kBarTogglePositionals[] = {
      {.name = "bar-name", .description = "target bar name", .required = false},
      {.name = "monitor-selector", .description = "target monitor selector", .required = false}
  };

  // msg brightness-set [monitor-selector] <value>
  constexpr CliPositional kBrightnessSetPositionals[] = {
  ...
  constexpr CliPositional kEffectsProfileSetPositionals[] = {
  ...
  constexpr CliPositional kNotificationPositionals[] = {
      {.name = "summary",
  ...
  // msg panel-close [id]
  constexpr CliPositional kPanelClosePositionals[] = {{.name = "id", .description = "panel id", .required = false}};

  // msg panel-open <id> [context]
  // msg panel-toggle <id> [context]
  constexpr CliPositional kPanelOpenTogglePositionals[] = {
  ...
  constexpr CliPositional kPluginPositionals[] = {
  ...
  constexpr CliPositional kPowerCyclePositionals[] = {
      {.name = "direction", .description = "optional direction", .required = false}
  };

  // msg plugins <action>
  constexpr CliPositional kPluginsPositionals[] = {
      {.name = "action", .description = "plugin management action", .required = true}
  };

  // msg power-set <profile>
  constexpr CliPositional kPowerSetPositionals[] = {
  ...
  constexpr CliPositional kMonitorSelectorOptionalPositionals[] = {
      {.name = "monitor-selector", .description = "target monitor selector", .required = false}
  };

  // msg session <action>
  constexpr CliPositional kSessionPositionals[] = {
  ...
  constexpr CliCommand kIpcSubcommands[] = {
      ...
      {.name = "plugins", .summary = "Manage plugins and sources", .positionals = kPluginsPositionals},
      {.name = "power-cycle",
  ...
  std::span<const cli_schema::CliCommand> getIpcSubcommands() { return kIpcSubcommands; }

  constexpr const char* kNotificationHelpText = "Usage: noctalia msg notification-show <summary> <body>...\n"
                                                "\n"
                                                "Shows a desktop notification.\n";

  extern constexpr CliCommand kNotificationCmd = {
      .name = "notification-show",
      .summary = "Show an internal Noctalia notification",
  ...
  extern constexpr CliCommand kMsgCmd = {
