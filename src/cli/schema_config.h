...
namespace noctalia::config {
  ...
  inline constexpr cli_schema::CliPositional kExportPositionals[] = {
      {.name = "mode", .description = "merged or full", .choices = kExportChoices, .required = false},
  };

  inline constexpr cli_schema::CliPositional kReplayPositionals[] = {
      {.name = "report.toml", .description = "support report to replay", .required = true},
  };

  inline constexpr cli_schema::CliFlag kReplayFlags[] = {
      {.longName = "--target", .description = "target directory", .takesValue = true, .required = true},
