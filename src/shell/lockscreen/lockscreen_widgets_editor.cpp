...
namespace {
  ...
  constexpr float kDefaultLockscreenAudioVisualizerAspectRatio = 240.0f / 96.0f;
  ...
  std::string nextLockscreenWidgetId(const LockscreenWidgetsSnapshot& snapshot) {
    ...
    return std::format("lockscreen-widget-{:016x}", nextCounter);
  }
...
void LockscreenWidgetsEditor::initialize(
...
void LockscreenWidgetsEditor::setExitRequestedCallback(std::function<void()> callback) {
  m_exitRequestedCallback = std::move(callback);
}

void LockscreenWidgetsEditor::open(const LockscreenWidgetsSnapshot& snapshot) {
  m_snapshot = snapshot;
  m_open = true;
...
LockscreenWidgetsSnapshot LockscreenWidgetsEditor::close() {
...
bool LockscreenWidgetsEditor::isOpen() const noexcept { return m_open; }

float LockscreenWidgetsEditor::widgetContentScale(const DesktopWidgetState& state) const {
...
void LockscreenWidgetsEditor::syncSurfaces() {
...
void LockscreenWidgetsEditor::createSurface(const WaylandOutput& output) {
  auto surfaceConfig = LayerSurfaceConfig{
      .nameSpace = "noctalia-lockscreen-widgets-editor",
...
LockscreenWidgetsEditor::OverlaySurface* LockscreenWidgetsEditor::findSurface(wl_surface* surface) {
...
LockscreenWidgetsEditor::OverlaySurface* LockscreenWidgetsEditor::findSurface(const std::string& outputName) {
  for (auto& overlay : m_surfaces) {
...
LockscreenWidgetsEditor::OverlaySurface* LockscreenWidgetsEditor::findSurfaceForWidget(const std::string& widgetId) {
  for (auto& overlay : m_surfaces) {
...
LockscreenWidgetsEditor::EditorWidgetView* LockscreenWidgetsEditor::findView(const std::string& id) {
  for (auto& overlay : m_surfaces) {
...
DesktopWidgetState* LockscreenWidgetsEditor::findWidgetState(const std::string& id) {
...
const DesktopWidgetState* LockscreenWidgetsEditor::findWidgetState(const std::string& id) const {
...
std::string LockscreenWidgetsEditor::effectiveOutputName(const DesktopWidgetState& state) const {
...
bool LockscreenWidgetsEditor::shouldSnap() const {
  return (m_snapshot.grid.visible != m_shiftHeld) && m_snapshot.grid.cellSize > 0;
}

void LockscreenWidgetsEditor::prepareFrame(OverlaySurface& surface, bool needsUpdate, bool needsLayout) {
...
void LockscreenWidgetsEditor::rebuildScene(OverlaySurface& surface) {
  surface.views.clear();
...
void LockscreenWidgetsEditor::updateSelectionVisuals(OverlaySurface& surface) {
  const auto selectedIt = surface.views.find(m_selectedWidgetId);
...
void LockscreenWidgetsEditor::applyViewState(
...
void LockscreenWidgetsEditor::updateViewTransforms(const std::string* relayoutWidgetId) {
...
void LockscreenWidgetsEditor::addWidget(const std::string& outputName, const std::string& type) {
  ...
  widget.id = nextLockscreenWidgetId(m_snapshot);
  ...
  if (widget.type == "audio_visualizer") {
    widget.settings.emplace("aspect_ratio", static_cast<double>(kDefaultLockscreenAudioVisualizerAspectRatio));
...
void LockscreenWidgetsEditor::removeSelectedWidget() {
  if (m_selectedWidgetId.empty()) {
    return;
  }
...
void LockscreenWidgetsEditor::toggleSelectedWidgetEnabled() {
  if (m_selectedWidgetId.empty()) {
    return;
  }
...
void LockscreenWidgetsEditor::sendSelectedWidgetToBack() {
  if (m_selectedWidgetId.empty()) {
    return;
  }
...
void LockscreenWidgetsEditor::bringSelectedWidgetToFront() {
  if (m_selectedWidgetId.empty()) {
    return;
  }
...
void LockscreenWidgetsEditor::startToolbarDrag(const std::string& outputName) {
...
void LockscreenWidgetsEditor::clampToolbarPosition(OverlaySurface& surface, float toolbarWidth, float toolbarHeight) {
...
void LockscreenWidgetsEditor::startInspectorDrag(const std::string& outputName) {
  OverlaySurface* surface = findSurface(outputName);
...
void LockscreenWidgetsEditor::clampInspectorPosition(
...
// buildInspector and applySettingChange are in lockscreen_widgets_editor_settings.cpp

void LockscreenWidgetsEditor::deferEditorMutation(std::function<void()> action) {
...
void LockscreenWidgetsEditor::requestExit() {
...
void LockscreenWidgetsEditor::startDrag(
...
void LockscreenWidgetsEditor::updateDrag() {
  if (m_drag.mode == DragMode::None) {
    return;
  }
...
void LockscreenWidgetsEditor::finishDrag() {
...
bool LockscreenWidgetsEditor::onPointerEvent(const PointerEvent& event) {
  if (!m_open) {
    return false;
  }
...
void LockscreenWidgetsEditor::onKeyboardEvent(const KeyboardEvent& event) {
  if (!m_open) {
    return;
  }
...
void LockscreenWidgetsEditor::onOutputChange() {
  if (!m_open) {
    return;
  }
  syncSurfaces();
...
void LockscreenWidgetsEditor::onSecondTick() {
...
void LockscreenWidgetsEditor::requestLayout() {
  for (auto& surface : m_surfaces) {
    if (surface->surface != nullptr) {
...
void LockscreenWidgetsEditor::requestRedraw() {
  for (auto& surface : m_surfaces) {
    if (surface->surface != nullptr) {
      surface->surface->requestRedraw();
    }
