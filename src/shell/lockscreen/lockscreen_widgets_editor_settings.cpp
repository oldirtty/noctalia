...
namespace {
  ...
  std::unique_ptr<Flex> makeToggleRow(
      ...
      LockscreenWidgetsEditor* editor
  ) {
    return makeRow(
        labelText,
        ui::toggle({
  ...
  std::unique_ptr<Flex> makeSliderRow(
      ...
      const Settings& s, LockscreenWidgetsEditor* editor
  ) {
    return makeRow(
        labelText,
        ui::slider({
  ...
  std::unique_ptr<Flex>
  makeFilePickerRow(std::string_view labelText, const std::string& key, LockscreenWidgetsEditor* editor) {
    return makeRow(
        labelText,
        ui::button({
  ...
  void addSettingsSection(
      Flex& content, const std::vector<settings::WidgetSettingSpec>& specs, const Settings& s,
      LockscreenWidgetsEditor* editor, std::string_view labelKey, bool separator
  ) {
  ...
  void addBackgroundSection(Flex& content, const Settings& s, LockscreenWidgetsEditor* editor) {
...
void LockscreenWidgetsEditor::applySettingChange(const std::string& key, WidgetSettingValue value) {
...
void LockscreenWidgetsEditor::buildInspector(
