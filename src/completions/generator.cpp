...
namespace noctalia::completions {
  ...
  namespace {
    ...
    void generateFishCommand(std::ostringstream& ss, const cli_schema::CliCommand& cmd, const std::string& cmdPath) {
      std::string exact_cond = std::format("__fish_noctalia_exact {}", cmdPath);
      std::string prefix_cond = std::format("__fish_noctalia_prefix {}", cmdPath);

      // Comandos folha (sem subcomandos) continuam completando flags/arquivos mesmo com argumentos extras
      std::string context_cond = cmd.subcommands.empty() ? prefix_cond : exact_cond;
      bool acceptsFiles = false;

      for (const auto& sub : cmd.subcommands) {
        ss << std::format(
            "complete -c noctalia -n '{}' -a '{}' -d '{}'\n", exact_cond, sub.name, escapeQuotes(sub.summary)
        );
      ...
      for (const auto& flag : cmd.flags) {
        ...
        ss << std::format(
            "complete -c noctalia -n '{}' {} -d '{}'\n", context_cond, flagOpt, escapeQuotes(flag.description)
        );
      }
      ...
      if (acceptsFiles) {
        ss << std::format("complete -c noctalia -n '{}' -F\n", context_cond);
      }
    }
  ...
  std::string generateFish(const cli_schema::CliCommand& root) {
    ...
    ss
        << "function __fish_noctalia_exact\n"
    ...
    ss
        << "function __fish_noctalia_prefix\n"
        << "    set -l cmd (commandline -opc)\n"
